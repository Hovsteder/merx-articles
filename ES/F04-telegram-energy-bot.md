# Construyendo un bot de Telegram que compra energia en TRON

Telegram is the de facto communication platform for the crypto community. If you manage TRON wallets, run a dApp, or simply want a convenient way to purchase energy without opening a browser, a Telegram bot is a practical tool. This tutorial walks through building a complete Telegram bot that checks precios de energia, purchases energy through MERX, and sends notifications when orders fill.

By the end of this article, you will have a working bot with three commands -- `/price`, `/buy`, and `/balance` -- plus webhook integration for tiempo real estado de la orden updates.

## Prerequisites

- Node.js 18 or later
- A Telegram bot token (from @BotFather)
- A MERX clave de API (from [https://merx.exchange](https://merx.exchange))
...
2. **MERX client** -- Interact with the mercado de energia
...
The `/price` command queries MERX for current precios de energia across all providers:
...
The webhook handler receives estado de la orden notifications from MERX and forwards them to the user via Telegram:
...
Extend the bot with a `/alert` command that uses MERX orden permanentes to notify users when prices drop below a threshold:
...
Wrap all command handlers with consistent manejo de errores:
...
4. Send `/price 65000` to check current precios de energia
5. Send `/buy 65000 1h TYourAddress` to place an order
6. Wait for the webhook notification confirming the order filled

## Conclusion

A Telegram bot for TRON compra de energia turns a web-based workflow into a conversational interface. With three core commands and webhook integration, users can check prices, buy energy, and receive fill notifications without leaving Telegram.

The implementation leverages the MERX SDK for all mercado de energia interactions, which means the bot automatically gets enrutamiento al mejor precio across seven providers, orden permanente support for price alerts, and reliable ejecucion de ordenes with respaldo.
