# Ordenes permanentes: automatice compras de energia en TRON las 24 horas

## The Manual Approach Does Not Scale

If you are buying TRON energy manually, you are doing it wrong. Not because the process is difficult - placing an order takes seconds - but because the mercado de energia is dynamic. Prices fluctuate throughout the day. Delegations expire on fixed schedules. Your application's energy needs change based on transaction volume. Managing all of this manually means either constant vigilance or wasted money.
...
This article covers the full orden permanente system: trigger types, action types, budget controls, and the practical patterns that save the most money.

## How Standing Orders Work

A orden permanente is a persistent rule stored in MERX's PostgreSQL database. It consists of three components:
...
The MCP server exposes orden permanentes through the `create_standing_order` tool:
...
This order will purchase 500,000 energy for 24 hours whenever the market price drops below 0.00005 TRX per unidad de energia, up to 3 times per day, spending no more than 100 TRX daily.

## Trigger Types

### price_below

Fires when the best available precio de energia drops below your specified threshold.
...
This is the most common trigger for optimizacion de costos. Energy prices on TRON fluctuate based on supply and demand. During off-peak hours (typically 00:00-06:00 UTC), prices often drop 15-25% below peak levels. A `price_below` trigger lets you automatically buy energy during these windows without monitoring prices yourself.

**Practical tip:** Set your threshold at the 25th percentile of the last 7 days' prices. Esto significa you buy when prices are in the bottom quarter of their recent range - cheap enough to save money, frequent enough to keep your address supplied.
...
**How it works:** MERX polls the en cadena resource balance of the specified address at regular intervals (every 60 seconds by default). When the available energy drops below the threshold, the trigger fires.

**Important caveat:** Hay inherent latency between the balance dropping and the new energy arriving (intervalo de sondeo + order placement + delegation confirmation). Set the threshold high enough that your address does not run out during this window. If your application consumes 50,000 energy per minute, a threshold of 100,000 gives you approximately 2 minutes of runway - sufficient for MERX to replenish.
...
This is the standard action for adquisicion de energia. MERX automatically selects the cheapest available provider at the time of execution. The `target_address` receives the delegated energy.
...
Send a notification without taking any en cadena action.
...
Use this when you want awareness without automation. The orden permanente monitors the condition and alerts you, but you make the purchase decision manually. This is a good starting point for teams that are not yet comfortable with fully automated purchases.
...
The maximum total TRX the orden permanente can spend in a rolling 24-hour period.
...
The orden permanente automatically deactivates after this timestamp.
...
Always set an expiration on orden permanentes. An orphaned orden permanente that runs indefinitely after you've forgotten about it is a liability.

### Total Budget

A hard cap on total spending across the lifetime of the orden permanente.
...
Once the lifetime spend reaches this limit, the orden permanente is permanently deactivated.
...
Permanently removes the orden permanente. Execution history is retained for audit purposes.
...
For AI agents, orden permanentes are the mechanism for persistent behavior. An agent session is temporary - when the MCP connection closes, the agent can no longer monitor prices or buy energy. Standing orders run on the MERX server, independent of any client connection.

An agent can set up orden permanentes during one session and benefit from them across all future sessions:
...
Your application runs 24/7. Your adquisicion de energia should too.
