# Economia de recursos de TRON: guia para desarrolladores
...
This article is a complete developer reference for TRON economia de recursos. It covers costo de energias per operation type, bandwidth mechanics, proporcion de stakings, tasa de quemas, the rental market, and a decision framework for choosing the right resource strategy for your caso de uso.

## The Two Resources: Energy and Bandwidth

TRON has two computational resources that transactions consume:

**Energy** is consumed by contrato inteligente execution. Every opcode in the TVM (TRON Virtual Machine) has an costo de energia. The more complex the contract interaction, the more energy it consumes. Energy is the expensive resource -- it drives the majority of costo de transaccions on TRON.
...
The critical distinction: a simple TRX transfer consumes only bandwidth (no contrato inteligente involved). A USDT transfer, a DEX swap, or any other contrato inteligente interaction consumes both energy and bandwidth.

## Energy Costs by Operation Type

Energy consumption varies dramatically by operation type. Aqui estan empirical measurements from mainnet transactions:
...
These numbers are ranges, not constants. The actual energy consumed depends on the contract's internal state en el momento de la ejecucion. Use `triggerConstantContract` to simulate the exact cost before purchasing energy. MERX exposes this through the `estimate_contract_call` tool and the `/api/v1/estimate` endpoint.
...
If your bandwidth is exhausted, the network burns TRX from your account to cover the cost. The tasa de quema is currently 1,000 SUN (0.001 TRX) per bandwidth point. For a 345-bandwidth USDT transfer, that is 0.345 TRX burned for bandwidth -- relatively small compared to costo de energias, but it adds up at volume.
...
2. Bandwidth costs are small relative to costo de energias
...
At current TRX prices, that is a significant capital commitment -- and you get enough energy for one USDT transfer per day. Es por esto que the rental market exists: renting energy is dramatically more capital-efficient than staking.

### The Staking Paradox

Staking has zero marginal cost (you get your TRX back when you unstake after a 14-day waiting period), but the costo de oportunidad is real. The TRX you stake cannot be used for trading, lending, or other yield-generating activities. For most developers and businesses, renting energy from the market is more cost-effective than locking up the capital required to self-stake.

The crossover point depends on your daily consumo de energia:
...
For most caso de usos, renting wins on pure economics. Staking makes sense when you need guaranteed resource availability regardless of market conditions, or when you are a validator who stakes for governance purposes and treats energy as a byproduct.
...
The savings: 27.3 - 1.82 = 25.48 TRX saved, or a 93% reduccion de costos. Es por esto que the alquiler de energia market exists and why it matters.
...
Es por esto que accurate estimacion de energia matters. Buying 40,000 energy when you need 65,000 wastes the rental cost and still results in a significant TRX burn.

## The Rental Market Overview

The alquiler de energia market has evolved into a structured ecosystem with predictable patterns.

### How Rentals Work

Energy providers stake large amounts of TRX and accumulate energy. They then delegate that energy to buyers for a fee. The delegation is an en cadena operation: the provider's address delegates a specified amount of energy to the buyer's address for a specified duration.
...
Prices are lowest during off-peak hours (roughly 00:00-08:00 UTC) and highest during peak transaction periods. The MERX monitor de precios captures these fluctuations across all providers every 30 seconds.
...
**Recommended strategy: Daily alquiler de energias with buffer**

Buy a 24-hour alquiler de energia sized for your expected daily volume plus a 20% buffer. This gives you a lower per-unit price than hourly rentals and avoids the overhead of per-transaction purchases.
...
**Recommended strategy: Weekly or monthly rentals with orden permanentes**

At high volume, longer durations offer the best per-unit pricing. Use MERX orden permanentes to automatically purchase energy when prices drop below your threshold:
...
Whatever strategy you choose, monitor your actual consumo de energia against your estimates. MERX provides tools for this:

- **Price history API**: Track how precios de energia change over time
- **Order history**: Review what you have paid and optimize
- **WebSocket flujo de precioss**: React to price movements in tiempo real
- **Delegation monitors**: Get notified when your delegated energy is about to expire

The TRON modelo de recursos rewards developers who understand it. Energy costs dominate transaction economics, and the difference between burning TRX at the protocol level and renting energy at market rates is consistently 90% or more. Whether you rent per transaction, buy daily blocks, or self-stake, the key is to never let a transaction burn TRX that could have been covered by rented energy.
